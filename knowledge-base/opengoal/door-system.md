...
- [ ] **starts_open**: Emits `perm-status` lump (value 4). Engine checks `(logtest? entity extra perm status complete)` at spawn — needs live test to confirm the lump is read correctly.
- [ ] **Trigger volume → sun-iris-door**: Volumes send `'notify`; `sun-iris-door` only handles `'trigger`. Use `basebutton` instead until this is fixed.
- [x] **`eco-door` etype crash** — FIXED: abstract base remaps to `jng-iris-door` at export time.
- [x] **`eco-door` flags wrong** — FIXED: `auto-close=4` (bit 2), `one-way=8` (bit 3). Was incorrectly 1 and 2.
- [x] `sun-iris-door.o` — confirmed correct in `SUN.DGO` (`sun.gd`).
- [x] `basebutton.o` — confirmed in `GAME.CGO` (always resident, no injection needed).
