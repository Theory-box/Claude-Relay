...
- **Blackbody** (and Volume Principled temperature) → two-stage: extract value
  manifest in source, re-apply in target after downgrade (the subtype value-loss
  class; the node exists in the target but its socket drops the value).
