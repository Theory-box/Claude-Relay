...
- MERGED feature/accumulate -> main (user approved, all in good state).
- IBP INVESTIGATION + REMOVAL: user reported Enhance only pixelates. Diagnosed (Node, realistic drizzle recon): IBP reduces rmse ~11% BUT amplifies grid/cell structure gridE +18% at ALL sigma (0.4-1.8) -> the pixelation, not a settings bug. Quick edge-preserving regularization made gridE worse. Root: deconvolving a grid-structured RGB reconstruction amplifies the grid; guessed PSF; modest ceiling (RGB not RAW). DECISION: removed (reversibly). SR_IBP_ENABLED=false gates srStoreFrame (no per-frame buffering overhead); Enhance UI wrapped in #srIbpUI display:none. srEnhanceIBP + code left dormant. TO REVERSE: SR_IBP_ENABLED=true + show #srIbpUI. Core super-res (kernel regression + robustness + native res + zoom-crop) is the real value and remains.
