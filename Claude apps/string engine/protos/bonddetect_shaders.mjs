...
struct BD { count:u32, gx:u32, gy:u32, cap:u32, cell:f32, ox:f32, oy:f32, er2:f32, candCap:u32, p0:u32, p1:u32, p2:u32, };
@group(0) @binding(0) var<storage, read>       pos        : array<vec4<f32>>;
@group(0) @binding(1) var<storage, read>       endList    : array<u32>;
...
@group(0) @binding(5) var<storage, read_write> cand       : array<vec2<u32>>;   // (n1,n2) node ids, n1<n2
@group(0) @binding(6) var<storage, read_write> candOver   : array<atomic<u32>>;
@group(0) @binding(7) var<uniform>             U          : BD;
@compute @workgroup_size(64)
fn main(@builtin(global_invocation_id) gid:vec3<u32>){
  let ei=gid.x; if(ei>=U.count){ return; }
  let n1=endList[ei]; let p1=pos[n1];
  let gxi=i32(U.gx)-1; let gyi=i32(U.gy)-1;
  let cx=clamp(i32(floor((p1.x-U.ox)/U.cell)),0,gxi);
  let cy=clamp(i32(floor((p1.y-U.oy)/U.cell)),0,gyi);
  for(var dy:i32=-1; dy<=1; dy=dy+1){
    let yy=cy+dy; if(yy<0 || yy>gyi){ continue; }
    for(var dx:i32=-1; dx<=1; dx=dx+1){
      let xx=cx+dx; if(xx<0 || xx>gxi){ continue; }
      ...
      for(var k:u32=0u; k<cnt; k=k+1u){
        ...
        if(n2<=n1){ continue; }                                  // dedup: each unordered pair once
        let p2=pos[n2]; let ddx=p2.x-p1.x; let ddy=p2.y-p1.y; let d2=ddx*ddx+ddy*ddy;
        if(d2<U.er2 && d2>1e-6){
