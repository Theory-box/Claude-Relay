...
## Critical: save/load now round-trips ALL settings + Circle.002 density finding

saveScene only serialized basic fields (ids/solid/fixed/color/thickness/stiffness/curl/grow/damp/padSelf/padOther/selfSolid/inter/interSelf) and 8 globals — DROPPING bonding (bondOn, endType profiles), breaking (breakable/brkStretch/brkAbs/bendAbsLim/brkRel/bendRelLim), autoSpace/spaceMult, affinity/polarity, and globals (bonding master, bondSnap/Brk/Cure/Blend/Timer, gCurl/gStiff/gThick/gGrow, chunk, attract/repel/tol, view). So saved scenes lost their whole grow->bond->break setup and couldn't be reproduced. Fixed: saveScene serializes all per-object fields + all globals; importJSON applies them (incl applyAutoSpace + computeExclusions). Verified round-trip: endType, bondOn, brkAbs/bendAbsLim, brkRel/bendRelLim, autoSpace, spaceMult, grow, bonding master, gGrow all preserved.

User's uploaded scene (scene-1784753880150) predates this fix -> Circle.002 has grow:0 and no bond/break config, so it can't reproduce the live grow/break/merge behavior. Circle.002 geometry finding: 483 edges, 48 (~10%) shorter than its 12.5px contact distance (thickness 2.5 + padSelf 7.5); ~38 self-overlapping non-adjacent pairs. Auto-space at spaceMult 1 barely helps (38->33); at 1.5 it makes it WORSE (33->53) because expanding a dense closed shape forces it to buckle -> more self-overlap. Conclusion: auto-space can't fix a shape that's simply too dense for its thickness+padding; needs thinner thickness/pad or decimated geometry. The spin/overlap dynamic needs the user to re-save with the fixed build so the real bond/break settings load.

---

## "Expand to fit" — live uniform scale-to-fit (replaces auto-space)

Replaced instant auto-space with a LIVE, gradual UNIFORM scale: each frame a relax-on object eases a per-object spaceScale toward objScaleNeeded = min(8, max(1, (2*effR+padSelf)*spaceMult / shortestAuthoredEdge)), then sets every edge rest = rest0*spaceScale. Scales the whole shape by one factor (proportions preserved -> varied meshes keep their look), so the shortest link reaches the padded-thickness contact distance. Grow composes cleanly (grow is a constraint-time multiplier on rest, doesn't touch base). Cap 8x so one degenerate tiny edge can't explode the shape. Exclusions recompute every 30 frames while relaxing. UI: General tab button relabeled "expand to fit"; thickness/padSelf/spaceMult changes now rely on the live force (gradual, no instant snap); toggle-off restores authored + spaceScale=1. spaceMult range 0.5-3.

Verified: a crammed strand inflates cleanly (56->164px, scale 2.75, 0 overlaps). Circle.002 cut short edges 44->14 but couldn't fully inflate — it FILLS THE CANVAS so the walls block expansion, and it's densely tangled (packing resists). Honest limit: expand-to-fit works when the shape has room; a canvas-filling tangled mesh needs decimation/thinner thickness or a bigger world. Save/load already round-trips autoSpace/spaceMult (spaceScale is derived live).
...
## Overlapping-clumps diagnosis: absolute bend-break + self-bonding churn

User scene (scene-1784757143853) Circle.002: negative grow -0.16, self-bond (wStr .05, merge 0 instant fuse, snap 3.5), brkAbs 90, weave .6, temp .7. Object shattered 128 -> 6710 pieces / 7193 nodes in 300 frames, clustering into ~338 tight clumps = the "2-3 overlapping strings". ISOLATION: brkAbs OFF -> 133 pieces (stable); bonding OFF -> 411; neither -> 128; no-grow -> still 7588. So the cause is **absolute bend-break + self-bonding**: self-bond fuses two ends, but merged ends usually meet <90deg, so brkAbs instantly snaps the joint -> ends re-bond -> re-snap -> shatter, fragments clump via attraction. FIX: **wouldBendBreak(a,b)** — before an instant fuse, compute the would-be joint angle at the merge midpoint; if <bendAbsLim, skip the fuse (mark nodes used). Cut fragmentation 6710 -> 1799. Verified straight bonds still form. Remaining churn (128->1799) is bend-break correctly snapping joints that flex past 90 during the agitated/bendy sim (weave/temp/neg-grow) then re-bonding — inherent to the settings; reduce via higher bend limit or less weave/temp. (Node count still creeps during churn = atomic-sever adds a node each break; the planned per-frame safety cap will bound the cost.)
